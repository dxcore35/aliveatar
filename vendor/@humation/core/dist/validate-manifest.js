export function validateManifest(manifest) {
    ...
    const selectionSlotIds = new Set(manifest.selectionSlots.map((slot) => slot.id));
    const uiGroupIds = new Set(manifest.uiGroups.map((group) => group.id));
    collectDuplicateIssues(manifest.selectionSlots.map((slot) => slot.id), 'selectionSlots', issues);
    collectDuplicateIssues(manifest.uiGroups.map((group) => group.id), 'uiGroups', issues);
    collectDuplicateIssues(manifest.layerSlots.map((slot) => slot.id), 'layerSlots', issues);
    collectDuplicateIssues(manifest.colors.map((color) => color.id), 'colors', issues);
...
function collectDuplicateIssues(values, path, issues) {
    const seen = new Set();
